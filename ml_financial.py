...
st.set_page_config(
    page_title="FinChat - Your Personal Financial Analyst",
...
def display_streamlit_updates():
    ...
    with st.expander("Streamlit 1.37 Announcement", expanded=False):
...
def initialize_conversation():
    ...
    """
    assistant_message = "Hello! I am FinBee. How can I assist you with finance today?"

    conversation_history = [
        {"role": "system", "content": "You are FinBee, a specialized AI assistant trained in Streamlit."},
        {"role": "system", "content": "FinBee, is powered by the OpenAI GPT-4o-mini model, released on July 18, 2024."},
        ...
        {"role": "system", "content": "You were created by Tobi Odeyemi, an OpenAI Researcher."},
...
def get_latest_update_from_json(keyword, latest_updates):
    """
    Fetch the latest finance update based on a keyword.

    Parameters:
    - keyword (str): The keyword to search for in the finance updates.
    - latest_updates (dict): The latest finance updates data.
...
def convert_to_dataframe(result):
    # Convert the string representation of the array to a list of dictionaries
...
@st.cache_data(show_spinner=False)
def on_finchat_submit(chat_input):
    """
    Handle chat input submissions and interact with the OpenBB API.

    Parameters:
    - chat_input (str): The chat input from the user.

    Returns:
    - None: Updates the response history in Streamlit's session state.
    """
    try:
        user_input = chat_input.strip().lower()
        if user_input == '':
...
def main():
    ...
    img_path = "imgs/3d_Bumble_Bee.jpg"
    ...
    mode = st.sidebar.radio("Select Mode:", options=[ "Ask FinBee"], index=0)
    ...
    if mode == "Chat with FinBee":
        chat_input = st.chat_input("Ask me about finance updates:")
        if chat_input:
    ...
    elif mode == "Ask FinBee":
        chat_input = st.text_input("Ask me for any Financial Analysis Assistance:","")
        if chat_input:
            on_finchat_submit(chat_input)
